...
function AppAppBar() {
  ...
  const noOfCartItem = findNoOfItems(cart) || findNoOfItems(JSON.parse(window.localStorage.getItem('cart')))
      ...
      <AppBar position="fixed">
        <Toolbar sx={{ display: 'flex', justifyContent: 'space-between', bgcolor: 'white', borderBottom: '2px solid #f5f5f5' }}>
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            ...
            >
              <Typography
                            sx={{ ml: 0, fontSize: 24, color: '#FF3366', textDecoration: 'none', paddingTop: '3px' }}
              >E-MART</Typography>
            </Link>
          </Box>
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
